...
export default function Comments({ postId }: CommentsProps) {
  ...
  const renderComment = (comment: Comment, depth: number = 0) => (
    <div key={comment.id} className={`${depth > 0 ? 'ml-8 mt-6' : 'mt-6'} ${depth === 0 ? 'border-b border-slate-200 dark:border-slate-700 pb-6' : ''}`}>
      <div className="flex items-start space-x-3">
        {/* 작은 아바타 */}
        <div className="flex-shrink-0">
          <div className="w-6 h-6 bg-gradient-to-br from-violet-500 to-pink-500 rounded-full flex items-center justify-center text-white font-medium text-xs">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* 댓글 내용 */}
          <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed whitespace-pre-wrap mb-3">
            {comment.content}
          </p>
        </div>
      </div>
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="ml-6 border-l border-slate-200 dark:border-slate-700 pl-4 mt-4">
    ...
    <div className="mt-8 border-t border-slate-200 dark:border-slate-700 pt-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <div className="w-6 h-6 bg-gradient-to-r from-violet-500 to-pink-500 rounded-md flex items-center justify-center">
        ...
        <button
          ...
          className="inline-flex items-center space-x-1 px-4 py-2 bg-gradient-to-r from-violet-500 to-pink-500 text-white text-sm rounded-lg hover:from-violet-600 hover:to-pink-600 focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 transition-all duration-200 shadow-sm"
        >
            ...
            {comments.length > INITIAL_COMMENT_COUNT && (
              <div className="text-center pt-6 border-t border-slate-200 dark:border-slate-700 mt-6">
                <button
                  ...
                  className="inline-flex items-center space-x-1 px-3 py-2 text-sm text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded transition-colors"
                >
                  {showAllComments ? (
                    <>
